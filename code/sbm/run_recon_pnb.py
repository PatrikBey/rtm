...
# of brain-region structure directly from continuous (0-100) per-       #
# subject lesion-load values, with no binarization/co-occurrence-       #
# counting step. Reconstruction is fit via plain single-flip             #
# mcmc_sweep() (NOT multiflip), which was found to be essential —       #
# multiflip search never finds structure for this model, while          #
# single-flip search reliably finds and keeps it (see                   #
# project_run_recon_ising_fit_investigation memory, "REVERSAL"          #
# section).                                                             #
#                                                                       #
# Two variants are fit: no_beh (raw lesion values) and beh_weighted     #
# (lesion values multiplied by a rescaled behaviour score before        #
...
args = argparse.ArgumentParser(description='Run PseudoNormalBlockState reconstruction directly on continuous lesion-load data.')
...
args.add_argument('--out_dir', type=str, default='/mnt/h/RT/data/RESULTS/SBMRECONPNB', help='Path to the output directory')
...
log_msg(f"| START | Running PseudoNormalBlockState reconstruction on continuous lesion-load data")
...
def region_behaviour_relevance(vals, behaviour):
    ...
    continuous lesion load here — Spearman is rank-based so this works
...
# raw_vals is rescaled to [0, 1] (global min-max) so it's on the same scale
# as the [0, 1]-rescaled behaviour score used for weighting below.
# beh_weighted: rescaled lesion values multiplied by the rescaled behaviour
# score (per subject) before reconstruction -- amplifies each subject's
# lesion pattern in proportion to their behaviour score, rather than adding
# behaviour as a separate correlated dimension (which was tested and found
# to never change the recovered structure relative to no_beh at all).
raw_vals_01      = rescale_01(raw_vals)
...
log_msg(f"| UPDATE | Fitting PseudoNormalBlockState with behaviour-weighted lesion values (single-flip search)")
...
log_msg(f"| UPDATE | Fit complete with behaviour-weighted lesion values ({len(np.unique(blocks_beh_weighted))} blocks)")
