...
#                                                                       #
# Surface visualisation of the Schaefer2018-400 atlas coloured by      #
# hemisphere-aware anatomical region (14 regions, same colour scheme   #
# as plot_tracts_nilearn.py).                                           #
#                                                                       #
# For each of the 400 atlas parcels the areas file supplies the        #
# anatomical region label; this is mapped to a 1-14 region index and   #
# projected onto the fsaverage5 surface.                                #
#                                                                       #
# Output: Schaefer2018-400_regions_surface.png                          #
#                                                                       #
# authors: Bey, Patrik                                                  #
#                                                                       #
...
# Load areas file: row i → label (col 0) and region (region col) for parcel i+1
...
# roi_regions[i] = hemisphere-sensitive region name for atlas parcel (i+1)
roi_regions = [
    f"{areas_data[i, region_col]}_{areas_data[i, 0].split('_')[-1]}"
    for i in range(len(areas_data))
]

# Build stable region → index mapping (1-based, order of first appearance)
seen = {}
region_index_map = {}
...
# roi_colors: region index → RGB (universal, covers all 14 regions)
roi_colors = {
...
out_path = os.path.join(data_path, 'ATLAS', f'{atlas}_regions_surface.png')
