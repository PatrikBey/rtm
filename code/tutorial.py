...
# appeared with different prior probabilities (p17, p33, p50, p67, p83).
# Higher prior probability → faster responses (higher promptness mean).

cw = pylater.data.cw1995   # dict keyed by "{participant}_{condition}"

print("=== Section 3: bundled cw1995 dataset keys ===")
print(list(cw.keys()))
print()

# Pick one participant, two probability conditions
d_p17 = cw["a_p17"]   # 17 % prior — slow
d_p50 = cw["a_p50"]   # 50 % prior — medium
d_p83 = cw["a_p83"]   # 83 % prior — fast

for d in [d_p17, d_p50, d_p83]:
...
#
# The log-probability of a promptness value r is:
#   log[ f_late(r) * Φ_early(r)  +  f_early(r) * Φ_late(r) ]
# This is a mixture-race: whichever process wins (max promptness) triggers the
# response.
...
ax1 = axes[0]
rp1 = pylater.ReciprobitPlot(ax=ax1)
...
ax2 = axes[1]
rp2 = pylater.ReciprobitPlot(ax=ax2)
...
plt.tight_layout()
plt.savefig("reciprobit_single.png", dpi=120)
...
#
# This captures the effect of prior probability: higher p → higher promptness
# mean → line shifts left (faster responses).

print("=== Section 10: shift model (parallel lines) ===")
model_shift = pylater.build_default_model(
    datasets    = [d_p17, d_p50, d_p83],
    share_type  = "shift",
)
...
comparison = az.compare({"shift": idata_shift, "swivel": idata_swivel})
